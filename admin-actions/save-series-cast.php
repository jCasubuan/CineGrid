...
for ($i = 0; $i < count($actors); $i++) {
    ...
    if ($actor === '') {
        continue; // Actor name is required
    }

    // Validate actor name using the validation function
    $actorValidation = isValidPersonName($actor, 'Actor');
    if (!$actorValidation['valid']) {
        http_response_code(422);
        echo json_encode(['error' => $actorValidation['error']]);
        exit;
    }

    // Validate character name (optional but if provided, must be valid)
    if ($character !== '') {
        $characterValidation = isValidCharacterName($character);
        if (!$characterValidation['valid']) {
            http_response_code(422);
            echo json_encode(['error' => $characterValidation['error']]);
            exit;
        }
    }
    ...
    $cleanCast[] = [
        'actor' => $actor,
        'character' => $character !== '' ? $character : 'Role TBA', // Default if empty
